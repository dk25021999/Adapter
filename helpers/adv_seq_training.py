...
def task_(model, data_loader, loss_func, optimizers, device, target_attribute='gender', gate_version=1):
    ...
    for ids, token_types, att_masks, labels, attributes in tqdm_bar:
        ...
        task_loss.backward()
        optimizers.step()
        # print(optimizers.param_groups[0]['lr'])
...
def adv_(model, attr_models, debias_method, data_loader, loss_func, optimizers,
    ...
    for ids, token_types, att_masks, labels, attributes in tqdm_bar:
        ...
        temperature = [temp_values[1]] * len(tgt_attribute)
        logits, embeds = model(ids, token_type_ids=token_types, attention_mask=att_masks,
        ...
        for count, packet in enumerate(zip(attributes, tgt_attribute)):
            ...
            if debias_method != 'dann':
                ...
                att_l, attr_info = attr_models[name].get_da_loss([
                    ...
                    att_l, attr_info = attr_models[name].get_da_loss([embeds[-1]], attribute)
            attr_loss.append(att_l)
            attr_loss_[name] = np.round((19 * attr_loss_[name] + att_l.item()) / 20, 3)

        total_loss = task_loss + sum(attr_loss)
        total_loss.backward()
        optimizers.step()
        optimizers.zero_grad()
...
def adv_seq_train(model, attr_models, attacker_model, debias_method, train_loader, validate_loader,
                    ...
                    train_type='model attacker', gate_version=1, sequence_training=1):
    ...
    if 'model' in train_type:
        if sequence_training:
            for i in range(train_epochs):
                print(f'Epoch:  {i + 1}')
                train_loss, train_acc = task_(model, train_loader, loss_func, optimizers,
                ...
                current_task_lr = optimizers.param_groups[0]['lr']
        ...
        if 'none' not in target_attribute:
            ...
            for i in range(train_epochs):
                ...
                train_loss, train_attr_loss, train_acc = adv_(model, attr_models, debias_method, train_loader,
                ...
                current_attr_lr = optimizers.param_groups[0]['lr']
